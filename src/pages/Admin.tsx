...
export default function Admin() {
  ...
  }, [authLoading, currentUserEmail, isAdmin]);
  ...
  const handleDelete = async (type: 'ads' | 'users', id: string) => {
    if (!window.confirm(`Delete this ${type === 'ads' ? 'listing' : 'user'}?`)) return;
    try {
  ...
  const handleRunAd = async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error("Upload failed. Check Firebase Storage rules and CORS.");
    } finally {
  ...
  if (authLoading) return <div className="p-10 text-center font-bold text-green-700 uppercase">Loading Dashboard...</div>;
  if (!isAdmin) return <div className="p-10 text-center text-red-500 font-bold">UNAUTHORIZED: NOT AN ADMIN</div>;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in duration-300">
            ...
            <div className="lg:col-span-5 bg-white p-6 rounded-3xl border shadow-xl h-fit">
              ...
              <div className="space-y-4">
                ...
                <input type="url" placeholder="Redirect Link" value={clientLink} onChange={(e) => setClientLink(e.target.value)} className="w-full p-4 bg-gray-50 rounded-xl border-none text-xs font-bold focus:ring-2 focus:ring-blue-500 outline-none" />
                ...
                <tbody className="divide-y divide-gray-50">
                  {activeTab === 'ads' ? ads.map(ad => (
                    <tr key={ad.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="p-5"><span className="text-green-600 font-black text-xs">Rs {Number(ad.price).toLocaleString()}</span></td>
                      <td className="p-5 text-right"><button onClick={() => handleDelete('ads', ad.id)} className="text-red-400 hover:text-red-600"><Trash2 size={18}/></button></td>
                    </tr>
                  )) : users.map(u => (
                    <tr key={u.uid} className="hover:bg-gray-50/50">
                      ...
                      <td className="p-5 text-right"><button onClick={() => handleDelete('users', u.uid)} className="text-red-400 hover:text-red-600"><Trash2 size={18}/></button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
